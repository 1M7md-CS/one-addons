...
class OneAddonsScreen : Screen(Component.literal("OneAddons")) {
    ...
    private fun onRender(g: ScreenGraphics, mouseX: Int, mouseY: Int, delta: Float) {
        ...
        // Panel shadow
        g.fill(cx - 2, cy - 2, cx + PANEL_W + 2, cy + PANEL_H + 2, C_SHADOW)
        // Outer border
        g.fill(cx, cy, cx + PANEL_W, cy + PANEL_H, C_BORDER)
        // Panel background
        g.fill(cx + 1, cy + 1, cx + PANEL_W - 1, cy + PANEL_H - 1, C_PANEL)

        // Title bar area
        g.fill(cx + 1, cy + 1, cx + PANEL_W - 1, cy + TITLE_BAR_H, C_TITLE_BG)
        g.fill(cx + 1, cy + TITLE_BAR_H, cx + PANEL_W - 1, cy + TITLE_BAR_H + 1, C_ACCENT)

        val title = Component.literal("\u00A7e\u26A1 OneAddons")
        g.text(font, title, width / 2 - font.width(title) / 2, cy + 8, C_TITLE, true)

        val tabY = cy + TITLE_BAR_H + 8
        val tabW = (PANEL_W - 2) / 3
        val tabs = arrayOf("\u2B21 Rift", "\u2726 Enchant", "\u2699 Utility")

        // Tab bar background
        g.fill(cx + 1, tabY, cx + PANEL_W - 1, tabY + TAB_H, C_TAB_BAR_BG)

        for (i in 0..2) {
            val tx = cx + 1 + i * tabW
            val hover = mouseX >= tx && mouseX < tx + tabW - 1 && mouseY >= tabY && mouseY < tabY + TAB_H
            val bg = if (i == currentTab) C_TAB_ACTIVE else if (hover) C_TAB_HOVER else C_TAB_BG
            g.fill(tx, tabY, tx + tabW - 1, tabY + TAB_H, bg)
            g.text(font, Component.literal(tabs[i]), tx + tabW / 2 - font.width(tabs[i]) / 2, tabY + 6, C_TITLE, true)

            // Active tab indicator
            if (i == currentTab) {
                g.fill(tx + 4, tabY + TAB_H - 2, tx + tabW - 5, tabY + TAB_H, C_ACCENT)
            }
        }
        ...
        val contentY = lineY + 8
        val contentBottom = cy + PANEL_H - 8

        g.enableScissor(cx + 1, contentY, cx + PANEL_W - 1, contentBottom)
        drawCurrentTab(g, cx, contentY - scrollOffset + 4, mouseX, mouseY)
        g.disableScissor()
        ...
        if (totalH > contentH) {
            ...
            val sbX = cx + PANEL_W - SCROLLBAR_W - 5
            g.fill(sbX, contentY, sbX + SCROLLBAR_W, contentBottom, C_SCROLLBAR_TRACK)
            g.fill(sbX, thumbY, sbX + SCROLLBAR_W, thumbY + thumbH, C_SCROLLBAR_THUMB)
        }
    }
    ...
    private fun drawEnchantTab(g: ScreenGraphics, cx: Int, y: Int, mx: Int, my: Int) {
        ...
        drawSectionHeader(g, left, right, y, "\u2728 Enchanting")

        var yy = y + HEADER_H

        drawToggleRow(g, left, right, yy, mx, my, "\uD83D\uDD2E Enchanting Assist",
    ...
    private fun drawUtilityTab(g: ScreenGraphics, cx: Int, y: Int, mx: Int, my: Int) {
        ...
        drawSectionHeader(g, left, right, yy, "\uD83D\uDEE0\uFE0F Utilities")
        yy += HEADER_H + 4
        ...
        val bindColor = if (capturingKey) C_ACCENT else C_KEYBIND
        val bindX = left + 22
        val bindY = yy + 2
        val bindW = font.width(bindText) + 8
        val bindH = 12
        val bindHover = !capturingKey && mx >= bindX && mx < bindX + bindW && my >= bindY - 1 && my < bindY + bindH + 1

        if (bindHover) g.fill(bindX - 2, bindY - 1, bindX + bindW + 2, bindY + bindH + 1, C_ROW_HOVER)
        g.fill(bindX, bindY, bindX + bindW, bindY + bindH, C_KEYBIND_BG)
        g.text(font, Component.literal(bindText), bindX + 3, bindY + 2, bindColor, true)
        yy += 16

        drawSeparator(g, left + 20, yy, right)
        yy += 8
        ...
        if (OneAddons.swapAssistEnabled) {
            ...
            for (i in entries.indices) {
                ...
                // Entry background
                g.fill(left + 2, rowY, right - 2, rowY + ROW_H, C_ENTRY_BG)

                val del = "\u2716"
                val delX = left + 6
                val delHover = mx >= delX && mx < delX + 10 && my >= rowY && my < rowY + ROW_H
                g.text(font, Component.literal(del), delX, rowY + 6, if (delHover) C_RED else C_DIM, true)

                val tog = if (e.enabled) "[\u2713]" else "[ ]"
                val togX = left + 18
                val togHover = mx >= togX && mx < togX + font.width(tog) && my >= rowY && my < rowY + ROW_H
                g.text(font, Component.literal(tog), togX, rowY + 6, if (togHover) C_ACCENT else if (e.enabled) C_SWITCH_ON else C_DIM, true)

                val tSlot = "Slot " + e.triggerSlot
                val tsX = togX + font.width(tog) + 6
                ...
                if (editing) g.fill(tsX - 2, rowY + 2, tsX + font.width(tSlot) + 2, rowY + ROW_H - 2, C_EDITING_BG)
                g.text(font, Component.literal(tSlot), tsX, rowY + 6, if (tsHover) C_ACCENT else C_ROW_TEXT, true)

                val tInt = if (e.triggerInteract) "[R]" else "[_]"
                val tiX = tsX + font.width(tSlot) + 6
                val tiHover = mx >= tiX && mx < tiX + font.width(tInt) && my >= rowY && my < rowY + ROW_H
                g.text(font, Component.literal(tInt), tiX, rowY + 6, if (tiHover) C_ACCENT else C_DIM, true)

                val arrow = " \u2192 "
                val arX = tiX + font.width(tInt)
                g.text(font, Component.literal(arrow), arX, rowY + 6, C_ARROW, true)
                ...
                if (sediting) g.fill(ssX - 2, rowY + 2, ssX + font.width(sSlot) + 2, rowY + ROW_H - 2, C_EDITING_BG)
                g.text(font, Component.literal(sSlot), ssX, rowY + 6, if (ssHover) C_ACCENT else C_ROW_TEXT, true)

                val sInt = if (e.targetInteract) "[R]" else "[_]"
                val siX = ssX + font.width(sSlot) + 6
                val siHover = mx >= siX && mx < siX + font.width(sInt) && my >= rowY && my < rowY + ROW_H
                g.text(font, Component.literal(sInt), siX, rowY + 6, if (siHover) C_ACCENT else C_DIM, true)
            ...
            val addX = left + 20
            val addH = 18
            val addHover = editingSlot == null && mx >= addX && mx < addX + font.width(addText) + 10 && my >= yy && my < yy + addH
            if (addHover) g.fill(addX - 2, yy, addX + font.width(addText) + 8, yy + addH, C_BUTTON_HOVER)
            g.fill(addX, yy, addX + font.width(addText) + 6, yy + addH, C_BUTTON_BG)
            g.text(font, Component.literal(addText), addX + 3, yy + 4, if (editingSlot != null) C_DIM else C_ACCENT, true)
            yy += addH + 2
        }
        ...
        if (OneAddons.placeOnPositionEnabled) {
            ...
            for (i in entries.indices) {
                ...
                if (psEditing) g.fill(psX - 2, rowY + 2, psX + font.width(pSlot) + 2, rowY + ROW_H - 2, C_EDITING_BG)
                g.text(font, Component.literal(pSlot), psX, rowY + 6, if (psHover) C_ACCENT else C_ROW_TEXT, true)

                val pInt = if (e.placeInteract) "[R]" else "[_]"
                val piX = psX + font.width(pSlot) + 6
                val piHover = mx >= piX && mx < piX + font.width(pInt) && my >= rowY && my < rowY + ROW_H
                g.text(font, Component.literal(pInt), piX, rowY + 6, if (piHover) C_ACCENT else C_DIM, true)
                ...
                if (rsEditing) g.fill(rsX - 2, rowY + 2, rsX + font.width(rSlot) + 2, rowY + ROW_H - 2, C_EDITING_BG)
                g.text(font, Component.literal(rSlot), rsX, rowY + 6, if (rsHover) C_ACCENT else C_ROW_TEXT, true)

                val rInt = if (e.restoreInteract) "[R]" else "[_]"
                val riX = rsX + font.width(rSlot) + 6
                val riHover = mx >= riX && mx < riX + font.width(rInt) && my >= rowY && my < rowY + ROW_H
                g.text(font, Component.literal(rInt), riX, rowY + 6, if (riHover) C_ACCENT else C_DIM, true)
            ...
            val addX = left + 20
            val addH = 18
            val addHover = editingPlaceField == null && mx >= addX && mx < addX + font.width(addText) + 10 && my >= yy && my < yy + addH
            if (addHover) g.fill(addX - 2, yy, addX + font.width(addText) + 8, yy + addH, C_BUTTON_HOVER)
            g.fill(addX, yy, addX + font.width(addText) + 6, yy + addH, C_BUTTON_BG)
            g.text(font, Component.literal(addText), addX + 3, yy + 4, if (editingPlaceField != null) C_DIM else C_ACCENT, true)
            yy += addH + 2
        }
        ...
        if (OneAddons.keyMakerEnabled) {
            ...
            g.text(font, Component.literal(modeStr), modeX, yy + 6, if (modeHover) C_ACCENT else C_ROW_TEXT, true)
            yy += ROW_H

            val editing = editingKeyMakerDelay
            val delayStr = if (editing)
                (if (editingKeyMakerDelayBuf.isEmpty()) "\u258D\u258D\u258D" else editingKeyMakerDelayBuf)
            else
            ...
            if (editing) g.fill(delayX - 2, yy + 2, delayX + font.width(delayStr) + 4, yy + ROW_H - 2, C_EDITING_BG)
            g.text(font, Component.literal(delayStr), delayX, yy + 6, delayColor, true)
            yy += ROW_H
        }
    }

    private fun drawSectionHeader(g: ScreenGraphics, left: Int, right: Int, y: Int, text: String) {
        g.fill(left, y + 2, right, y + HEADER_H - 2, C_SECTION_BG)
        g.text(font, Component.literal(text), left + 4, y + 6, C_SECTION_TEXT, true)
    }

    private fun drawSeparator(g: ScreenGraphics, left: Int, y: Int, right: Int) {
    ...
    private fun drawToggleRow(g: ScreenGraphics, left: Int, right: Int, y: Int, mx: Int, my: Int, label: String, getter: BooleanSupplier, setter: Consumer<Boolean>) {
        ...
        g.text(font, Component.literal(label), left + 4, y + 6, C_ROW_TEXT, true)

        val swX = right - SWITCH_W - 4
        val swY = y + (ROW_H - SWITCH_H) / 2
        val on = getter.asBoolean
        val bg = if (on) C_SWITCH_ON else C_SWITCH_OFF
        val knobX = if (on) swX + SWITCH_W - SWITCH_KNOB_W else swX

        // Switch track
        g.fill(swX, swY, swX + SWITCH_W, swY + SWITCH_H, C_SWITCH_TRACK)
        g.fill(swX + 1, swY + 1, swX + SWITCH_W - 1, swY + SWITCH_H - 1, bg)
        // Switch knob
        g.fill(knobX + 1, swY + 1, knobX + SWITCH_KNOB_W - 1, swY + SWITCH_H - 1, C_SWITCH_KNOB)
    }
    ...
    private fun drawCategory(g: ScreenGraphics, cx: Int, y: Int, mx: Int, my: Int, name: String, vararg toggles: Toggle) {
        ...
        drawSectionHeader(g, left, right, yy, name)
        yy += HEADER_H + 4
    ...
    private fun onMouseClicked(mx: Int, my: Int, button: Int): Boolean {
        ...
        when (currentTab) {
            1 -> {
                clickEnchantTab(mx, my, cx, contentY - scrollOffset + 4)
                return true
            }
            2 -> {
                clickUtilityTab(mx, my, cx, contentY - scrollOffset + 4)
                return true
            }
        }

        clickCategoryTab(mx, my, cx, contentY - scrollOffset + 4)
        return false
    }
    ...
    private fun computeContentHeight(): Int {
        return when (currentTab) {
            0 -> HEADER_H + 4 + 2 * ROW_H + 10
            1 -> HEADER_H + 4 + ROW_H + 10
            else -> {
                var h = 0
                h += HEADER_H + 4 + ROW_H + 16
                h += 8 + ROW_H
                h += 8 + ROW_H
                if (OneAddons.swapAssistEnabled) {
                    h += OneAddons.swapAssistModule.entries.size * ROW_H + 4 + 18 + 2
                }
                h += 8 + ROW_H
                if (OneAddons.placeOnPositionEnabled) {
                    h += OneAddons.placeOnPositionModule.entries.size * ROW_H + 4 + 18 + 2
                }
                h += 8 + ROW_H
                if (OneAddons.keyMakerEnabled) {
                    h += ROW_H + ROW_H
    ...
    private fun clickUtilityTab(mx: Int, my: Int, cx: Int, y: Int) {
        ...
        val bindX = left + 22
        val bindW = font.width(bindText) + 8
        if (inRect(mx, my, bindX - 2, yy + 1, bindW + 4, 14)) {
        ...
        if (OneAddons.swapAssistEnabled) {
            ...
            if (inRect(mx, my, addX - 2, yy, addW + 4, 18)) {
    ...
    companion object {
        // ── Core palette ──────────────────────────────────────────────
        private val C_OVERLAY: Int = 0x99000000.toInt()
        private val C_SHADOW: Int = 0x66000000.toInt()
        private val C_BORDER: Int = 0xFF2E2E3E.toInt()
        private val C_PANEL: Int = 0xFF16161E.toInt()
        private val C_TITLE_BG: Int = 0xFF1E1E2A.toInt()

        // ── Tabs ──────────────────────────────────────────────────────
        private val C_TAB_BAR_BG: Int = 0xFF12121A.toInt()
        private val C_TAB_BG: Int = 0x00000000
        private val C_TAB_HOVER: Int = 0x20FFFFFF
        private val C_TAB_ACTIVE: Int = 0x30FFFFFF

        // ── Text ──────────────────────────────────────────────────────
        private val C_TITLE: Int = 0xFFFFFFFF.toInt()
        private val C_SECTION_TEXT: Int = 0xFF9B8FF0.toInt()
        private val C_ROW_TEXT: Int = 0xFFDDDDDD.toInt()
        private val C_DIM: Int = 0xFF7A7A8A.toInt()
        private val C_KEYBIND: Int = 0xFFB0B0C0.toInt()
        private val C_ARROW: Int = 0xFF5A5A6A.toInt()

        // ── Accents ────────────────────────────────────────────────────
        private val C_ACCENT: Int = 0xFF8B7EF0.toInt()
        private val C_RED: Int = 0xFFFF5555.toInt()

        // ── Surfaces ───────────────────────────────────────────────────
        private val C_SECTION_BG: Int = 0x1A8B7EF0
        private val C_SEPARATOR: Int = 0xFF2E2E3E.toInt()
        private val C_ROW_HOVER: Int = 0x20FFFFFF
        private val C_ENTRY_BG: Int = 0x0DFFFFFF
        private val C_EDITING_BG: Int = 0x308B7EF0
        private val C_KEYBIND_BG: Int = 0xFF22222E.toInt()

        // ── Switches ──────────────────────────────────────────────────
        private val C_SWITCH_ON: Int = 0xFF2FE3C9.toInt()
        private val C_SWITCH_OFF: Int = 0xFF3A3B4A.toInt()
        private val C_SWITCH_TRACK: Int = 0xFF101016.toInt()
        private val C_SWITCH_KNOB: Int = 0xFFFFFFFF.toInt()

        // ── Buttons ────────────────────────────────────────────────────
        private val C_BUTTON_BG: Int = 0xFF22222E.toInt()
        private val C_BUTTON_HOVER: Int = 0x308B7EF0

        // ── Scrollbar ──────────────────────────────────────────────────
        private val C_SCROLLBAR_TRACK: Int = 0x20FFFFFF
        private val C_SCROLLBAR_THUMB: Int = 0x60FFFFFF

        // ── Layout ─────────────────────────────────────────────────────
        private const val PANEL_W = 270
        private const val PANEL_H = 310
        private const val TITLE_BAR_H = 28
        private const val TAB_H = 22
        private const val HEADER_H = 22
        private const val PAD = 14
        private const val ROW_H = 24
        private const val SWITCH_W = 32
        private const val SWITCH_H = 13
        private const val SWITCH_KNOB_W = 11
